...
public class PurgeDatabaseAndCacheCmd implements Command<PurgeReport>, Serializable {
  ...
  private DatabasePurgeReport purgeDatabase(CommandContext commandContext) {
    ...
    //for mysql and maria db we have to disable foreign key check to delete bulk table data (execution, incident etc.)
    ...
    for (String tableName : tablesNames) {
      ...
      if (!TABLENAMES_EXCLUDED_FROM_DB_CLEAN_CHECK.contains(tableNameWithoutPrefix)) {
        ...
        if (!objects.isEmpty()) {
          ...
          if (entities.isEmpty()) {
            throw new ProcessEngineException("No corresponding DbEntity class was found for: " + tableName);
          }
